 ...
 * http://polymer.github.io/PATENTS.txt
 */
(window.litHtmlVersions||(window.litHtmlVersions=[])).push("1.1.2");const U=(t,...e)=>new f(t,e,"html",z),M=133;
/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
 ...
 * subject to an additional IP rights grant found at
 * http://polymer.github.io/PATENTS.txt
 */function V(t,e){const{element:{content:o},parts:s}=t,i=document.createTreeWalker(o,M,null,!1);let r=q(s),a=s[r],n=-1,p=0;const c=[];let l=null;for(;i.nextNode();){n++;const t=i.currentNode;for(t.previousSibling===l&&(l=null),e.has(t)&&(c.push(t),null===l&&(l=t)),null!==l&&p++;void 0!==a&&a.index===n;)a.index=null!==l?-1:a.index-p,a=s[r=q(s,r)]}c.forEach(t=>t.parentNode.removeChild(t))}const O=t=>{let e=11===t.nodeType?0:1;const o=document.createTreeWalker(t,M,null,!1);for(;o.nextNode();)e++;return e},q=(t,e=-1)=>{for(let o=e+1;o<t.length;o++){const e=t[o];if(u(e))return o}return-1};
/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
...
- url: /community_plugin/${this.repository.full_name.split("/")[1]}/${this.repository.file_name}
  type: ${void 0!==this.repository.javascript_type?U`${this.repository.javascript_type}`:U`${this.hass.localize("component.hacs.repository.lovelace_no_js_type")}`}</pre>
    ...
    >
          ...
          `}SetFeStyleGrid(){this.hass.connection.sendMessagePromise({type:"hacs/settings",action:"set_fe_grid"}).then(t=>{this.configuration=t,console.log(this.configuration),window.location.reload()},t=>{console.error("Message failed!",t)})}SetFeStyleTable(){this.hass.connection.sendMessagePromise({type:"hacs/settings",action:"set_fe_table"}).then(t=>{this.configuration=t,window.location.reload()},t=>{console.error("Message failed!",t)})}ReloadData(){this.ActiveSpinnerReload=!0,console.log("This should reload data, but that is not added.")}UpgradeAll(){this.ActiveSpinnerReload=!0,console.log("This should reload data, but that is not added.")}static get styles(){return[gt,ct`
    ha-card {
      width: 90%;
      margin-left: 5%;
    }
